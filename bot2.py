...
async def handle_start(message):
    if get_user(message.chat.id) == False:
...
async def handle_proxy(message):
    ...
    if proxy_url.startswith('http'):
        return await bot.send_message(message.chat.id, "Error! Use format: `User:Pass@IP:Port`")
...
async def handle_check_proxy(message):
    ...
    if proxy_settings:
        ...
        await bot.send_message(message.chat.id, "set proxy sar.")
...
async def handle_file_upload(message):
    ...
    with open("combos.txt", "r") as f:
        ...
        for card in cards:
            ...
            result = str(await call_checker(card))

            if result and 'Live' in result:
                with open("hits.txt", "a", encoding="utf-8") as hitfile:
                    hitfile.write(f"{result}\n")
            elif result.lower() != "dead" or result.lower() != None:
                None
            else:           
                await bot.send_message(message.chat.id, result)
